...
middleware.canUser = function (action) {
  return function (req, res, next) {
    if (!req.user) return res.status(401).json({ success: false, error: 'Not Authorized for this API call. error 1' });
    const permissions = require('../permissions');
    const perm = permissions.canThis(req.user.role, action);
    if (perm) return next();

    return res.status(401).json({ success: false, error: 'Not Authorized for this API call. error 2' });
  };
};
